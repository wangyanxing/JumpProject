...
void EffectSprite::draw(Renderer *renderer, const Mat4 &transform, uint32_t flags) {
  ...
  if (_insideBounds) {
    ...
    for (auto &effect : _effects) {
      ...
      q.init(_globalZOrder, _texture->getName(), std::get<1>(effect)->getGLProgramState(),
             _blendFunc, &_quad, 1, transform, flags);
    ...
    _trianglesCommand.init(_globalZOrder, _texture->getName(), getGLProgramState(), _blendFunc,
    ...
    for (auto it = std::begin(_effects) + idx; it != std::end(_effects); ++it) {
      ...
      q.init(_globalZOrder, _texture->getName(), std::get<1>(*it)->getGLProgramState(), _blendFunc,
             &_quad, 1, transform, flags);
      renderer->addCommand(&q);
      idx++;
    }
  }
...
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
  _backgroundListener = EventListenerCustom::create(
