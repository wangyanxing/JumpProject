...
Point rotateVector2(const Point&vec,float A){//A is degree
    Point rs;
...
float PNDisPointToLine(const Point&point,const Point&start,const Point&end){
    ...
    //        point
    //
    Point sp=point-start;
    Point se=end-start;
    Point se_e=ccpNormalize(se);
...
Point getIntersectPointOfTwoLine(const Point&p1,const Point&dir1, const Point&p2,const Point&dir2){
    ...
    if(D==0)return Point(INFINITY,INFINITY);
...
//otherwise the return vector will contain three element (p0,p1,p2)
{
    ...
    vector<Point> plist;
    Point vlist[3]={v0,v1,v2};
    for(int i=0;i<3;i++){
        //calculate p[i] (cur p)
        Point v=vlist[i];//cur v
        Point vn=vlist[(i+1)%3];//next v
        Point vf=vlist[(i-1+3)%3];//foe v
        Point dir_vvn=ccpNormalize(vn-v);
        Point dir_vvf=ccpNormalize(vf-v);
        Point dir_vp=ccpNormalize(dir_vvn+dir_vvf);//direction of vp
        ...
        Point vp=ccpMult(dir_vp, len_vp);
        Point p=v+vp;//got p
        plist.push_back(p);
...
Point getRightNormal(const Point&dir) {//right side normal or outer normal
    Point normal(dir.y,-dir.x);
    return normal;
}

Point getRightVector(const Point&p1,const Point&p2){//right side vector or outer vector, but not guarantee to be normalized
    Point vec=p2-p1;
    Point rightVec(vec.y,-vec.x);
    return rightVec;
...
{
    Point tangentPointLeft,tangentPointRight;
    ...
    Point dirPC=Point(PC.x/disPC,PC.y/disPC);
    
    Point dirPTLeft=rotateVector2(dirPC, cosA, sinA);
    Point dirPTRight=rotateVector2(dirPC, cosA, -sinA);
