...
void CrippleSprite::doTouch(const Point&touchPoint,float depth,float r){
    //touchPoint is in parent space, convert it to local space
    AffineTransform parentToNodeTransform=this->parentToNodeTransform();
    Point touchPoint_localSpace=PointApplyAffineTransform(touchPoint,parentToNodeTransform);
  ...
  //  cout<<"range:"<<imin<<" "<<imax<<" "<<jmin<<" "<<jmax<<endl;
    ...
    for(int i=imin;i<=imax;i++){
        for(int j=jmin;j<=jmax;j++){
            ...
            float dis=ccpDistance(Point(x_OLU,y_OLU), Point(v_OLU.x(),v_OLU.y()));
            if(dis<r){
...
void CrippleSprite::drawWire(){
    ...
    for(int i=0;i<nIDtri;i++){
        ...
        Point vertices[]={Point(v0.x(),v0.y()),Point(v1.x(),v1.y()),Point(v2.x(),v2.y())};
